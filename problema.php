<?php
    ...
    if($method == "POST") {
        ...
        function gerarUuid() {
            ...
            return vsprintf('%s%s%s%s-%s%s-%s%s-%s%s-%s%s%s%s%s%s', str_split(bin2hex($data),1));
        }
        ...
        $verifica = $conn->query("SELECT id FROM api_usuarios WHERE email='$email'");
        ...
        if ($conn->query($sql)) {
            $id = $conn->insert_id;
            $result = $conn->query("SELECT id,id,nome,email,telefone,endereco,estado,data_nascimento,criado_em
